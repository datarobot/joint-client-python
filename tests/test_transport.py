...
def test_transport_posts_json_with_headers_timeout_and_user_agent() -> None:
    ...
    result = transport.post_json(
        "https://example.com/predict", {"schema_version": "v3"}
    )
    ...
    assert json.loads(request_body.decode("utf-8")) == {"schema_version": "v3"}
...
def test_transport_retries_retryable_server_responses() -> None:
    ...
    try:
        ...
        result = transport.post_json(_server_url(server), {"schema_version": "v3"})
...
def test_transport_raises_status_error_when_gateway_html_persists() -> None:
    ...
    try:
        ...
        with pytest.raises(JointFMHTTPStatusError) as exc_info:
            transport.post_json(_server_url(server), {"schema_version": "v3"})
